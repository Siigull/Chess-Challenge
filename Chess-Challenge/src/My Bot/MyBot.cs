...
public class MyBot : IChessBot
{
    ...
    int[] pestos_eval = new int[1536];
    
    public MyBot() {
        int[,] mg_eg_value = {{ 95, 300, 310, 477, 1025,  0}, { 105, 290, 330, 512,  936,  0}};
    ...
    int search(int depth, int alpha, int beta, bool root, Timer timer) {
        ...
        for(int i=0; i<moves.Length; i++) {
            ...
            else if(moves[i].IsCapture) sorted[i] = ((int)(7 + moves[i].CapturePieceType - moves[i].MovePieceType), i).ToTuple();
    ...
    public Move Think(Board b, Timer timer)
    {
        ...
        for(int i=1; i<100; i++)
            if(search(i, -100000000, 100000000, true, timer) == 10000000) break;
